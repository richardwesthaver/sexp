...
* Abstract
This document defines the SXP data format. It is intended for
...
Rosetta Code]].
** Tokens
We explicitly define tokens as demonstrated in the implementation
section. SXPs are intended to be read and written from a stream of
bytes, so it makes sense to have as low-level of a representation of
individual characters in an S-expression as possible.
*** ListStart
The list starting character is typically =(=.
*** ListEnd
The List ending character is typically =)=.
*** COMMENT KeySymbol
A character which designates the start of a slot 'key' symbol,
typically =:=.
*** COMMENT ReaderSymbol
A character which designates that the next character is a 'read'
symbol, typically =#=.
*** COMMENT BackQuote
*** COMMENT Quote
*** Symbol
Just a plain unquoted string, such as =foo=. 
*** String
A string which prints with quotes, ="foo"=
*** Number
A string which contains numeric characters (Potential Number)
** Atoms
...
The reference implementation is located in the =rust= folder of this
project. It leverages the serde ecosystem and includes a full
