...
*** ListStart
The list starting character, typically =(=
*** ListEnd
The List ending character, typically =)=
*** Dot
A single dot character, =.=
*** COMMENT SlotSymbol
A character which designates the start of a 'slot' symbol
*** COMMENT ReaderSymbol
A character which designates that the next character is a 'read' symbol
*** Symbol
Just a plain unquoted string, =foo=
*** String
